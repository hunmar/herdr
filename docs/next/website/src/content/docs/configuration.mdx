...
To include agents from another machine in the local Agents sidebar, add a read-only source for its running Herdr session:
...
Herdr keeps one non-interactive SSH observer per source and reads the remote session snapshot every two seconds. Up to 32 sources can be configured. The remote machine must run Linux or macOS, have a protocol-compatible Herdr on `PATH` or in a standard install location, and already have the selected session running. Connect normally with `ssh devbox` or `herdr --remote devbox` first to accept the host key and configure authentication; background observers never prompt for passwords or host-key approval. Remote rows are display-only in this first version. When a connection drops, the last known rows remain visible as offline, and local agent shortcuts continue to target local panes only.

Named sessions are shown as `<label>/<session>` so two sessions on the same machine remain distinguishable. Fleet labels must be unique.

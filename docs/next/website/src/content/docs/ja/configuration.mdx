...
別のマシンで実行中の Herdr セッションにいるエージェントをローカルの Agents サイドバーへ表示するには、読み取り専用ソースを追加します。

```toml
[[remote.agent_sources]]
target = "devbox"       # OpenSSH の設定エイリアスまたは user@host
label = "devbox"        # 省略可能。既定値は target
session = "default"
```

Herdr はソースごとに非対話 SSH オブザーバーを 1 つ維持し、2 秒ごとにリモートセッションのスナップショットを読み取ります。最大 32 ソースまで設定できます。リモートマシンは Linux または macOS で、プロトコル互換の Herdr が `PATH` または標準のインストール先にあり、対象セッションがすでに実行中である必要があります。最初に通常の `ssh devbox` または `herdr --remote devbox` で接続し、ホストキーの承認と認証設定を済ませてください。バックグラウンドのオブザーバーはパスワードやホストキー承認を求めません。この初期バージョンではリモート行は表示専用です。接続が切れると最後に確認した行をオフラインとして残し、ローカルのエージェントショートカットは引き続きローカルペインだけを対象にします。

名前付きセッションは `<label>/<session>` として表示されるため、同じマシン上の複数セッションも区別できます。フリート内の表示ラベルは一意である必要があります。
